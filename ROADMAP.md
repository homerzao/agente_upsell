...
- [x] **Proibir correção de CIDADE / UF / CEP** — muda custo de frete. Camadas:
...
- [x] **Auto-arquivar conversa finalizada** — sweeper arquiva conversa liberada pro SAC
      há 30+ min sem correção pendente. Lista principal fica só com o realmente aberto.
- [x] **Aba Aprovações legível** — sai o JSON cru; entra tabela campo a campo com
...
- [x] Schema: `ofertas` ganha `ticket_min`, `ticket_max`, `prioridade`, `sku_gatilho`
      (gatilho por SKU no carrinho fica pronto no schema, ainda sem uso).
- [x] Elegibilidade: na hora do disparo escolhe a oferta pela faixa; sem faixa que case → não dispara.
- [x] Oferta nova: **PROTETOR FPS 70 CLAREADOR (SKU 2080) a R$ 19,90** para pedidos < R$ 50.
      Oferta atual (Kit Clareador 2133823 a R$ 49,91) para pedidos ≥ R$ 50.
- [x] Copies próprias da oferta nova (o copies já é por oferta — ok).
- [x] Painel: config das faixas visível/editável.
- [x] Contrato do faturamento intocado: a resposta já devolve `sku` dinâmico — o ERP não muda.
...
- [x] Tela extra de CONFIRMAÇÃO após o aceite: resume o que a pessoa leva
...
- [x] Textos do flow ficam DINÂMICOS via data_exchange (produto/preço vêm da oferta da faixa) —
      necessário pro multi-oferta usar o MESMO flow.
- [x] Montar em DRAFT + preview (flow 936615756148123, zero erros de validação) pro Jorge avaliar ANTES de publicar (publicado é imutável).
- [ ] Template novo apontando pro flow v8 (só depois do OK dele; cutover amanhã).
- [x] Métrica nova: quantos passam (evento ticket_quero; painel na fase do visual) do aceite-1 e desistem no double-check (mede o clique acidental).
...
- [x] Hoje TUDO já é escopado por `store` (coluna em todas as tabelas) — a fundação existe.
- [x] Tabela `empresas` (migration 015; hidrabene semeada, config vazia = envs) (tenant): nome, slug, credenciais Meta/TechSAC/Yampi/Pagar.me próprias,
      ativo. `disparos_config` e `ofertas` passam a ser por empresa.
- [ ] Painel: seletor de empresa no topo; usuários por empresa (fase 2).
- [x] Documento de arquitetura do multi-tenant (docs/multi-tenant.md): o que falta pra plugar a empresa nº 2
